...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImage.src = "images/pizza-med.svg";
...
var resizePizzas = function(size) { 
  ...
  //OPTIMIZE: Move out of determineDX function - reduced 50ms
  ...
  function changePizzaSizes(size) {
    //OPTIMIZE: Set pointer to '.randomPizzaContainer' outside of loop
    var pizzaContainer = document.querySelectorAll(".randomPizzaContainer");
    //OPTIMIZE: Determine size of pizza container outside of loop as all containers are the same
    var dx = determineDx(pizzaContainer[0], size);
    //OPTIMIZE: Calculate new width outside loop once, as all containers are the same
...
function updatePositions() {
  ...
  //OPTIMIZE: Determine scroll top position outside of for loop - reduced 20ms last 10 frames
  var scrollPosition = document.body.scrollTop / 1250;

  for (var i = 0; i < items.length; i++) {
  var phase = Math.sin(scrollPosition + (i % 5));   
    //items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
    //OPTIMIZE: Utilize transform style, avoiding layout re-rendering
    //OPTIMIZE: Initial position of pizza is set outside of loop
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //OPTIMIZE: Reduce number of sliding pizzas needed from 200 to 36
  for (var i = 0; i < 36; i++) {
    ...
    //OPTIMIZE: Set initial position of pizzas. Enable transformX from initial position
