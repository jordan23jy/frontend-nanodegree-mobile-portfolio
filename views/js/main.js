...
// Returns the size of pizza in each case. Called by changePizzaSlices(size).
// Optimize using scale to change the size of pizzas, avoiding restructuring of DOM
...
// Iterates through pizza elements on the page and changes their size
var x = document.querySelectorAll('.randomPizzaContainer img');
function changePizzaSizes(size) {
    for (i = 0; i < x.length; i++) {
      x[i].style.transform = newsize;
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 6; i++) {
    ...
    elem.style.left = (i % cols) * s * 4.2 + 70 + 'px';
    elem.style.top = (Math.floor(i / cols) * s + 100) + 'px';
    document.querySelector("#movingPizzas1").appendChild(elem);
  }
  document.querySelector("#movingPizzas1").display = 'block';
  //updatePositions();
});
